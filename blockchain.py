...
class Block:
    ...
    def calculateHash(self):
        return hashlib.sha256(str(self.index).encode('utf-8') + self.previousHash.encode('utf-8') + self.timestamp.encode('utf-8') + json.dumps(self.data).encode('utf-8')).hexdigest()
...
def main():
    Coin = BlockChain()
    Coin.addBlock(Block(1, "10/10/2017", {"account": "Monica","amount": 25,"action": "buy"}))
    Coin.addBlock(Block(2, "11/01/2017", {"account": "Joe","amount": 10,"action": "buy"}))
    Coin.addBlock(Block(3, "12/01/2017", {"account": "Rachel","amount": 20,"action": "buy"}))
    Coin.addBlock(Block(4, "12/07/2017", {"account": "Ross","amount": 4,"action": "buy"}))
    Coin.printBlockChain()
    # no tampering in our block chain yet so should be true here
    print ("Chain valid? " + str(Coin.isChainValid()))
    # now lets tamper the block chain and see what happens
    Coin.chain[1].data = {"account": "Monica","amount": 100,"action": "buy"}
    print ("Chain valid? " + str(Coin.isChainValid()))
